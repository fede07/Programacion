...
#include "PersonaException.h"
                        ...
                        ///esto es mas efectivo
Persona::Persona() /**: dni(0), apyn(), sexo('M'), fNac(), estadoCivil('S')**/
{
    this->setDni(0);
	this->setApyn() ;
	this->setSexo('M');
...
{
	///TODO: aplicar trim -> PROBABLEMENTE sea normalizar
...
///of = escritura (output file)
void Persona::write(ofstream& stream) const
{
    ///en stream copia n bytes (segundo parametro) de dni (primer parametro)
