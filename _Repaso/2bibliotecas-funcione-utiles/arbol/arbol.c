...
///la llama el main
void cargarDesdeDatosPreOrdenArbol(t_arbol * arbol, const void * datos, unsigned tam, unsigned cantDatos, int(*comparar)(const void *, const void *))
{
    int cantElemInsertados = 0;
    cargarDesdeDatosPreOrdenArbolRec(arbol,datos,tam,cantDatos,cantElemInsertados,comparar);
}
///complementa a cargarDesdeDatosPreOrdenArbol
int cargarDesdeDatosPreOrdenArbolRec(t_arbol * arbol, const void * datos, unsigned tam, unsigned cantDatos, unsigned cantElemInsertados, int(*comparar)(const void *, const void *))
{
    if(cantElemInsertados == cantDatos)
        return cantElemInsertados;

    insertarArbolRec(arbol,(void*)datos,tam,comparar);
    cantElemInsertados+=1;

    cantElemInsertados = cargarDesdeDatosPreOrdenArbolRec(arbol,datos+tam,tam,cantDatos,cantElemInsertados,comparar);

    return cantElemInsertados;
}
///la llama el main
void cargarDesdeArchPreOrdenArbol(t_arbol * arbol, FILE * arch, unsigned tam, const void * datos, int(*comparar)(const void *, const void *), void(*accion)(const void*, const void*) )
{
    char cadena[tam];
    ...
    {
        accion(cadena,&datos);
        insertarArbolRec(arbol,&datos,tam,comparar);
    }

}
///complementa a cargarDesdeArchPreOrdenArbol
///void insertarProd(const void* cadena, const void* datos)
///{
///    sscanf((char*)cadena,"%d,%d",&((tmovimiento*)datos)->cod,&((tmovimiento*)datos)->cant);
///}
...
void eliminarElementosArbolNoClave (t_arbol * arbol, const void * filtro, int(*comparar)(const void *, const void *))///RID
{
    if(!*arbol)
        return;

    eliminarElementosArbolNoClave(&(*arbol)->izq,filtro,comparar);
    eliminarElementosArbolNoClave(&(*arbol)->der,filtro,comparar);

    if(comparar((*arbol)->info,filtro))
        eliminarNodo(arbol);
}
...
///1 AVL / 2 balanceado / 3 completo / 0 ninguno / -1 no tiene nodos
...
int contarNodosConFiltro (const t_arbol * arbol, int (*fFiltro)(const void* a))
{
    if(!*arbol)
        return 0;

    if( fFiltro((*arbol)->info) )
        return contarNodosConFiltro (&(*arbol)->izq,fFiltro) + contarNodosConFiltro (&(*arbol)->der,fFiltro) + 1;

    return contarNodosConFiltro (&(*arbol)->izq,fFiltro) + contarNodosConFiltro (&(*arbol)->der,fFiltro);
}
...
///arbol
void acumularDatos (t_arbol * arbol, const void * acum, int fFiltro(const void* a),void* (*acumular)(void * acum, void *n))
{
    if(!*arbol)
        return;

    if( fFiltro((*arbol)->info) )
        acumular((void*)acum,(*arbol)->info);

    acumularDatos(&(*arbol)->izq,acum,fFiltro,acumular);
    acumularDatos(&(*arbol)->der,acum,fFiltro,acumular);
}

void promCondicionado (t_arbol * arbol, void * acum, void * prom, int fFiltro(const void* a),void* (*acumular)(void * acum, void *n), void (*promedio)(void * prom, const void * acum, const void * cant))
{
    int cantNodos;
    cantNodos = promDatosCond(arbol,acum,fFiltro,acumular);
    promedio(prom, acum,&cantNodos);
}

int promDatosCond (t_arbol * arbol, void * acum, int fFiltro(const void* a),void* (*acumular)(void * acum, void *n))
{
    if(!*arbol)
        return 0;
    ...
    {
        acumular((void*)acum,(*arbol)->info);
        return  promDatosCond(&(*arbol)->izq,acum,fFiltro,acumular) + promDatosCond(&(*arbol)->der,acum,fFiltro,acumular) + 1;
    }

    return promDatosCond(&(*arbol)->izq,acum,fFiltro,acumular) + promDatosCond(&(*arbol)->der,acum,fFiltro,acumular);
}
